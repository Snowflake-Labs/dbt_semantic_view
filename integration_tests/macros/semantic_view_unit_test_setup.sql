...
{# Route project refs through the package's unit-test-aware wrapper. #}
{% macro ref() %}
  {{ return(dbt_semantic_view.sv_aware_ref(varargs, kwargs)) }}
{% endmacro %}


{# Route project sources through the package's unit-test-aware wrapper. #}
{% macro source(source_name, table_name) %}
  {{ return(dbt_semantic_view.sv_aware_source(source_name, table_name)) }}
{% endmacro %}


{# Share the semantic-view body between dbt run and unit tests. #}
{% macro sv_def__semantic_view_basic_for_unit_test() %}
